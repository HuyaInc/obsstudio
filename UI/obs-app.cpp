...
static bool MakeUserDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "obs-studio-ex/basic") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "obs-studio-ex/logs") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "obs-studio-ex/profiler_data") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

#ifdef _WIN32
	if (GetConfigPath(path, sizeof(path), "obs-studio-ex/crashes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "obs-studio-ex/updates") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif

	if (GetConfigPath(path, sizeof(path), "obs-studio-ex/plugin_config") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
static bool MakeUserProfileDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "obs-studio-ex/basic/profiles") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "obs-studio-ex/basic/scenes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
bool OBSApp::InitGlobalConfig()
{
	...
	int len = GetConfigPath(path, sizeof(path), "obs-studio-ex/global.ini");
	if (len <= 0) {
		return false;
	}
...
bool OBSApp::SetTheme(std::string name, std::string path)
{
	...
	if (path == "") {
		...
		string temp = "obs-studio-ex/" + name;
...
static void move_basic_to_profiles(void)
{
	...
	if (GetConfigPath(path, 512, "obs-studio-ex/basic") <= 0)
		return;
	if (!os_file_exists(path))
		return;

	/* if the profiles directory doesn't already exist */
	if (GetConfigPath(new_path, 512, "obs-studio-ex/basic/profiles") <= 0)
		return;
...
static void move_basic_to_scene_collections(void)
{
	...
	if (GetConfigPath(new_path, 512, "obs-studio-ex/basic/scenes") <= 0)
		return;
	if (os_file_exists(new_path))
		return;
...
bool OBSApp::OBSInit()
{
	...
	//NimoPC:子窗口化
	if (NimoObsProxy::Instance()->m_hContainer != nullptr) {
		WId parentWId = reinterpret_cast<WId>(
			NimoObsProxy::Instance()->m_hContainer);

		QWindow *parentWnd = QWindow::fromWinId(parentWId);

		mainWindow->setProperty("_q_embedded_native_parent_handle", /**< from QT source*/
					QVariant(parentWId));
		mainWindow->setWindowFlag(Qt::FramelessWindowHint);
		mainWindow->winId();
		mainWindow->windowHandle()->setParent(parentWnd);


		//TODO 1.reset the size and the position
...
static void create_log_file(fstream &logFile)
{
	...
	get_last_log(false, "obs-studio-ex/logs", lastLogFile);
#ifdef _WIN32
	get_last_log(true, "obs-studio-ex/crashes", lastCrashLogFile);
#endif

	currentLogFile = GenerateTimeDateFilename("txt");
	dst << "obs-studio-ex/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file(false, "obs-studio-ex/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("obs-studio-ex/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		program.AppInit();
		delete_oldest_file(false, "obs-studio-ex/profiler_data");
	...
	run:
		...
		NimoObsProxy::Instance()->Init(main);*/
...
static void main_crash_handler(const char *format, va_list args, void *param)
{
	...
	string crashFilePath = "obs-studio-ex/crashes";
...
{
	char path[512];
	int pathlen = GetConfigPath(path, 512, "obs-studio-ex/basic/profiles");
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		...
		} else if (arg_is(argv[i], "--hwnd", nullptr)) {
			if (++i < argc) {
				QString str(argv[i]);				
				NimoObsProxy::Instance()->m_hWnd = (HWND)str.toUInt();				
			} 
		} else if (arg_is(argv[i], "--hContainer", nullptr)) {
			if (++i < argc) {
				QString str(argv[i]);				
				NimoObsProxy::Instance()->m_hContainer = (HWND)str.toUInt();
			} 
		}
