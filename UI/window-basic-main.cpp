...
static void AddExtraModulePaths()
{
	...
	BPtr<char> config_bin =
		os_get_config_path_ptr("obs-studio-ex/plugins/%module%/bin");
	BPtr<char> config_data =
		os_get_config_path_ptr("obs-studio-ex/plugins/%module%/data");
	...
	: OBSMainWindow(parent), ui(new Ui::OBSBasic)
{
	...
	// NimoPC:
	NimoObsProxy::Instance()->SendObsReady((qint64)winId());
	...
	// NimoPC: Remove Menubar && Cancel Size Changed
	...
	// NimoPC:
	NimoObsProxy::Instance()->m_pObsBasic = this;

	/*m_heartTimer = new QTimer(this);
	connect(m_heartTimer, SIGNAL(timeout()), this, SLOT(OnCall_HeartTimer()));
	m_heartTimer->start(10000);*/
}

//void OBSBasic::OnCall_HeartTimer()
//{
//	NimoObsProxy::Instance()->SendHeart();
//}
...
void OBSBasic::CreateDefaultDisplayCaptureSource(OBSScene scene)
{
	const char *v_id = obs_get_latest_input_type_id("monitor_capture");
	obs_source_t *source = obs_source_create(
		v_id, "Default Monitor Capture", NULL, nullptr);
	...
	if (source) {
		...
		auto addSource = [](void *data_, obs_scene_t *scene_) {
			AddSource *data = (AddSource *)data_;
			obs_sceneitem_t *sceneitem;

			sceneitem = obs_scene_add(scene_, data->source);
...
void OBSBasic::CreateDefaultScene(bool firstStart)
{
	...
	//NimoPC:增加默认视频采集源
	CreateDefaultDisplayCaptureSource(scene);
...
retryScene:
	...
	//NimoPC:创建默认源
	if (obs_get_source_num(sources, OBS_SOURCE_TYPE_INPUT) == 0) {
		CreateDefaultDisplayCaptureSource(GetCurrentScene());
	}
...
void OBSBasic::OBSInit()
{
	...
	ret = snprintf(fileName, 512, "obs-studio-ex/basic/scenes/%s.json",
		       sceneCollection);
	if (ret <= 0)
...
// MimoPC: 接受WM_COPYDATA消息
bool OBSBasic::nativeEvent(const QByteArray &eventType, void *message, long *result)
{
	MSG *msg = (MSG *)message;
	if (msg->message == WM_COPYDATA) {
		COPYDATASTRUCT *pStruct = (COPYDATASTRUCT *)msg->lParam;
		QString sData = (char *)pStruct->lpData;
		NimoObsProxy::Instance()->DoNativeMsgProc(sData);
	} else if (msg->message == WM_DESTROY || msg->message == WM_NCDESTROY) {
	...
	} else if (msg->message == WM_CREATE || msg->message == WM_SHOWWINDOW) {
...
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "obs-studio-ex/logs") <= 0)
		return;
...
{
	UploadLog("obs-studio-ex/logs", App()->GetCurrentLog());
}

void OBSBasic::on_actionUploadLastLog_triggered()
{
	UploadLog("obs-studio-ex/logs", App()->GetLastLog());
}
...
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "obs-studio-ex/crashes") <= 0)
		return;
...
{
	UploadLog("obs-studio-ex/crashes", App()->GetLastCrashLog());
}
...
void OBSBasic::StreamingStart()
{
	...
	if (service_obj != NULL) {
		sServer = obs_service_get_url(service_obj);
	}

	NimoObsProxy::Instance()->SendStreamingStart(sServer);
}
...
void OBSBasic::StreamingStop(int code, QString last_error)
{
	...
	case OBS_OUTPUT_ERROR:
		...
		sError = "Output.ConnectFail.Error";
		break;
	...
	case OBS_OUTPUT_DISCONNECTED:
		...
		sError = "Output.ConnectFail.Disconnected";
	}
	...
	// NimoPC: 在这里通知停止推送
	NimoObsProxy::Instance()->SendStreamingStop(code, sError);
...
void OBSBasic::on_streamButton_clicked()
{
	if (outputHandler->StreamingActive()) {
		...
		// NimoPC: 点击停止
		NimoObsProxy::Instance()->SendClickStopStream();
		...
		if (service_obj != NULL) {
			sServer = obs_service_get_url(service_obj);
			sKey = obs_service_get_key(service_obj);
		}

		// 视频比特率
		int vbit = config_get_uint(basicConfig, "SimpleOutput", "VBitrate");		
		// 音频比特率
		int abit = config_get_uint(basicConfig, "SimpleOutput", "ABitrate");
		// 编码器
		const char *sEncoder = config_get_string( basicConfig, "SimpleOutput", "StreamEncoder");
		uint32_t fps = config_get_uint(basicConfig, "Video", "FPSInt");
		uint32_t cx = config_get_uint(basicConfig, "Video", "BaseCX");
		uint32_t cy = config_get_uint(basicConfig, "Video", "BaseCY");
		uint32_t ocx = config_get_uint(basicConfig, "Video", "OutputCX");
		uint32_t ocy = config_get_uint(basicConfig, "Video", "OutputCY");
		
		//NimoObsProxy::Instance()->SendClickStartStream(audioBitrate, videoBitrate, streamEnc, sServer, sKey);
		NimoObsProxy::Instance()->SendClickStartStream(
			fps, cx, cy, ocx, ocy, abit, vbit, sEncoder, sServer, sKey);
...
{
	char path[512];
	int ret = GetConfigPath(path, 512, "obs-studio-ex");
	if (ret <= 0)
		return;
...
int OBSBasic::GetProfilePath(char *path, size_t size, const char *file) const
{
	...
	ret = GetConfigPath(profiles_path, 512, "obs-studio-ex/basic/profiles");
	if (ret <= 0)
		return ret;
...
void OBSBasic::on_autoConfigure_triggered()
{
	...
	//test.exec();
	//NimoPC
	config_set_string(Config(), "SimpleOutput", "RecEncoder", "x264");

	config_set_string(Config(), "Output", "Mode", "Simple");
	config_set_string(Config(), "SimpleOutput", "RecQuality", "Stream");
	config_set_int(Config(), "Video", "BaseCX", 1920);
	config_set_int(Config(), "Video", "BaseCY", 1080);
	config_set_int(Config(), "Video", "OutputCX", 1280);
	config_set_int(Config(), "Video", "OutputCY", 720);

	config_set_uint(Config(), "Video", "FPSType", 0);
	config_set_string(Config(), "Video", "FPSCommon", "60");
